...
const modules: { name: string; href: string; icon: typeof LayoutDashboard; color: string; module: AppModule }[] = [
  ...
  { name: 'Économat', href: '/inventory', icon: Warehouse, color: 'bg-amber-600', module: 'inventory' },
...
export default function AppLayout() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100">
      ...
      <main className={`flex-1 overflow-auto ${(location.pathname.startsWith('/inventory') || location.pathname.startsWith('/recipes') || location.pathname.startsWith('/products') || location.pathname.startsWith('/production') || location.pathname.startsWith('/replenishment') || location.pathname.startsWith('/warehouse')) ? '' : 'p-6'}`}>
        <Outlet />
      </main>
