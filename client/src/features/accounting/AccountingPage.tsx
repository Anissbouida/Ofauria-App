...
/* ═══════════════════════ CONSOMMATION MATIERES TAB ═══════════════════════ */
/**
 * Vue PHYSIQUE (pas tresorerie) des matieres sorties de stock pour la periode.
 * Independante du flux cash : montre ce que tu as REELLEMENT consomme/perdu,
 * peu importe quand (ou si) la facture fournisseur a ete payee.
 *
 * Source : inventory_transactions outflows (quantity_change < 0)
 *   - 'production' / 'usage' : utilise en production
 *   - 'waste' : pertes / dlc / casse
 *   - 'adjustment' : ajustements manuels (inventaire physique negatif)
 *
 * Affichage : 3 cartes totaux par type, puis tableau detaille par ingredient
 * groupe par categorie avec sous-totaux. Tri par cout decroissant.
 */
type ConsumptionRow = {
...
const MOVEMENT_LABELS: Record<string, { label: string; color: string; bg: string }> = {
  production: { label: 'Production', color: '#0e7c3a', bg: '#d4edda' },
  usage: { label: 'Production', color: '#0e7c3a', bg: '#d4edda' },
  waste: { label: 'Perte', color: '#b71c1c', bg: '#f8d7da' },
  adjustment: { label: 'Ajustement', color: '#6c757d', bg: '#e9ecef' },
};
...
function ConsommationTab() {
  ...
  // Groupage par categorie d'ingredient, puis par ingredient
  // Chaque ingredient peut avoir plusieurs types (production + waste par ex.)
  // -> on cumule cout + qty par ingredient, on stocke aussi le breakdown par type
  ...
  return (
    <>
      ...
      <div className="odoo-alert" style={{ fontSize: '0.75rem' }}>
        <strong>Note tresorerie :</strong> cette vue montre les matières REELLEMENT consommees (sorties de stock physique).
        Elle est <strong>independante du cash</strong> : peu importe que la facture fournisseur soit deja payee ou pas, ce qui
        compte ici c'est ce qui a quitte ton stock. Le cout est calcule avec le prix unitaire actuel de chaque ingredient.
      </div>
      ...
      {isLoading ? (
        ...
        <div style={{ padding: 40, textAlign: 'center', color: 'var(--theme-text-muted)', border: '1px dashed var(--theme-bg-separator)', borderRadius: 4 }}>
          Aucune consommation enregistree pour {periodLabel}.
        </div>
      ) : (
            ...
            return (
              <div key={cat.name} style={{ border: '1px solid var(--theme-bg-separator)', borderRadius: 4, overflow: 'hidden' }}>
                <button onClick={() => setExpandedCats(s => ({ ...s, [cat.name]: !isOpen }))}
                  ...
                  <span style={{ color: 'var(--theme-text-muted)' }}>({cat.ingredients.length} ingredient{cat.ingredients.length > 1 ? 's' : ''})</span>
                  <div style={{ flex: 1 }} />
                  ...
                  <table className="odoo-table" style={{ margin: 0, borderTop: '1px solid var(--theme-bg-separator)' }}>
                    <thead>
                      <tr>
                        ...
                        <th style={{ textAlign: 'right', width: 110 }}>Qte sortie</th>
                        <th style={{ textAlign: 'right', width: 110 }}>Cout unitaire</th>
