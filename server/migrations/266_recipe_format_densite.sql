-- Migration 266 : Densite optionnelle sur le poids de format (audit A4, P1)
--
-- POURQUOI
--   La mig 162 convertit ml -> g avec densite 1 codee en dur (« approximation
--   standard en patisserie »), alors que la mig 227 a introduit la densite par
--   ingredient (`ingredients.densite_kg_l`). Mais le poids d'un format n'est
--   pas celui d'un ingredient — c'est celui d'une PATE / APPAREIL / mix, dont
--   la densite est propre au format (un cake dense n'a pas la meme que la creme).
--   Une creme a 1,05 kg/l ou un sirop a 1,30 kg/l faussent le prorata des
--   co-produits (ratio_poids) tant que la densite est figee a 1.
--
--   On ajoute donc densite_kg_l NULLABLE sur recipe_formats. Nullable = pas
--   renseigne = comportement actuel (densite 1). Renseigne = utilise dans les
--   3 vues qui convertissent quantite_par_format_unite (v_recipe_format_summary,
--   v_recipe_format_cost, v_plan_item_rendement).
--
--   Impact : coûts d'un format en g/kg -> inchange (le facteur reste 1 ou 1000).
--   Formats en ml/l avec densite non renseignee -> inchange (COALESCE(...,1)).
--   Formats en ml/l avec densite renseignee -> ecart proportionnel a densite.
--
-- PORTEE
--   + colonne recipe_formats.densite_kg_l
--   + refonte des 3 vues (v_recipe_format_summary, v_recipe_format_cost,
--     v_plan_item_rendement) pour appliquer COALESCE(densite_kg_l, 1) dans
--     la conversion ml/l -> g. Toute la logique restante est identique.
--
-- INVERSION
--   ALTER TABLE recipe_formats DROP COLUMN densite_kg_l;
--   Restaurer les 3 vues telles quelles depuis la mig 175.

ALTER TABLE recipe_formats
  ADD COLUMN IF NOT EXISTS densite_kg_l NUMERIC(6,3)
    CHECK (densite_kg_l IS NULL OR densite_kg_l > 0);

COMMENT ON COLUMN recipe_formats.densite_kg_l IS
  'Densite de la pate/appareil pour ce format (kg/l = g/ml). NULL = defaut 1. Applique par les vues quand quantite_par_format_unite est ml ou l.';
       ...
       COALESCE(SUM(
         rf.quantite_par_format_g
           * CASE rf.quantite_par_format_unite
               ...
               WHEN 'ml'::text THEN COALESCE(rf.densite_kg_l, 1)        -- Mig 266
               WHEN 'l'::text  THEN COALESCE(rf.densite_kg_l, 1) * 1000 -- Mig 266
               ELSE 1
             END::numeric
       ...
       COALESCE(vtw.total_weight_kg, 0::numeric) - COALESCE(SUM(
         rf.quantite_par_format_g
           * CASE rf.quantite_par_format_unite
               WHEN 'g'::text  THEN 1
               WHEN 'kg'::text THEN 1000
               WHEN 'ml'::text THEN COALESCE(rf.densite_kg_l, 1)
               WHEN 'l'::text  THEN COALESCE(rf.densite_kg_l, 1) * 1000
               ELSE 1
             END::numeric
           * rf.nb_par_defaut::numeric
...
COMMENT ON VIEW v_recipe_format_summary IS
  'Sommaire poids alloue aux formats actifs. mig 266 : densite du format appliquee en conversion ml/l -> g (defaut 1).';

-- ─── 2. v_recipe_format_cost + v_plan_item_rendement (dependante, drop CASCADE) ───
  ...
  SELECT rf.id, rf.recipe_id, rf.contenant_id,
         ...
         (rf.quantite_par_format_g
            * CASE rf.quantite_par_format_unite
                WHEN 'g'  THEN 1
                WHEN 'kg' THEN 1000
                WHEN 'ml' THEN COALESCE(rf.densite_kg_l, 1)        -- Mig 266
                WHEN 'l'  THEN COALESCE(rf.densite_kg_l, 1) * 1000 -- Mig 266
                ELSE 1
              END
...
COMMENT ON VIEW v_recipe_format_cost IS
  'mig 266 : densite du format appliquee en conversion ml/l -> g (defaut 1). Filtre is_active mig 175 conserve.';
...
WITH plan_format AS (
  SELECT ppi.id AS plan_item_id, ppi.plan_id, ppi.product_id, ppi.format_id,
         ...
         rf.densite_kg_l AS format_densite,          -- Mig 266
...
SELECT rr.plan_item_id, rr.plan_id, rr.product_id, rr.recipe_id, rr.format_id, rr.format_nom,
       ...
       CASE WHEN rr.format_id IS NOT NULL THEN
         rr.quantite_par_format_g * CASE rr.quantite_par_format_unite
           WHEN 'g' THEN 1
           WHEN 'kg' THEN 1000
           WHEN 'ml' THEN COALESCE(rr.format_densite, 1)        -- Mig 266
           WHEN 'l' THEN COALESCE(rr.format_densite, 1) * 1000  -- Mig 266
           ELSE 1
         END * rr.planned_quantity
...
COMMENT ON VIEW v_plan_item_rendement IS
  'mig 266 : densite du format appliquee en conversion ml/l -> g (defaut 1). Recreee car dependance de v_recipe_format_cost.';
