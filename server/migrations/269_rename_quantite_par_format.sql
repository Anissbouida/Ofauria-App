-- Migration 269 : Renommage quantite_par_format_g -> quantite_par_format (audit A8, P2)
--
-- POURQUOI
--   quantite_par_format_g contient depuis la mig 162 des kg / ml / l selon
--   quantite_par_format_unite. Le suffixe _g est trompeur ; chaque nouveau
--   lecteur doit lire le COMMENT pour ne pas se tromper. On renomme en
--   quantite_par_format (sans unite implicite dans le nom).
--
--   La colonne suit sa propre convention (quantite_par_format_unite dit dans
--   quelle unite lire la valeur). Les 3 vues qui projettent la colonne sont
--   refondues avec le nouveau nom ; le comportement reste identique.
--
-- PORTEE
--   ALTER TABLE recipe_formats RENAME COLUMN + DROP/CREATE des 3 vues
--   (v_recipe_format_summary, v_recipe_format_cost, v_plan_item_rendement),
--   identiques a la mig 266 sauf le nom de la colonne.
--   Aucune donnee modifiee.
--
-- INVERSION
--   ALTER TABLE recipe_formats RENAME COLUMN quantite_par_format TO quantite_par_format_g;
--   + restaurer les 3 vues telles quelles depuis la mig 266.

ALTER TABLE recipe_formats
  RENAME COLUMN quantite_par_format_g TO quantite_par_format;

COMMENT ON COLUMN recipe_formats.quantite_par_format IS
  'Quantite d''une unite de ce format, exprimee dans l''unite quantite_par_format_unite. Renomme en mig 269 (audit A8) : l''ancien nom _g etait trompeur car la colonne accepte g/kg/ml/l.';
...
format_lines AS (
  SELECT rf.id, rf.recipe_id, rf.contenant_id,
         rf.quantite_par_format, rf.quantite_par_format_unite,
         ...
         (rf.quantite_par_format
            * CASE rf.quantite_par_format_unite
                WHEN 'g'  THEN 1
                WHEN 'kg' THEN 1000
                WHEN 'ml' THEN COALESCE(rf.densite_kg_l, 1)
                WHEN 'l'  THEN COALESCE(rf.densite_kg_l, 1) * 1000
                ELSE 1
              END
...
format_costs AS (
  SELECT fl.id, fl.recipe_id, fl.contenant_id, fl.quantite_par_format, fl.quantite_par_format_unite,
...
SELECT fc.id, fc.recipe_id, fc.contenant_id, fc.quantite_par_format, fc.quantite_par_format_unite,
...
WITH plan_format AS (
  SELECT ppi.id AS plan_item_id, ppi.plan_id, ppi.product_id, ppi.format_id,
         ...
         rf.quantite_par_format, rf.quantite_par_format_unite, rf.nb_par_defaut,
         rf.densite_kg_l AS format_densite,
...
SELECT rr.plan_item_id, rr.plan_id, rr.product_id, rr.recipe_id, rr.format_id, rr.format_nom,
       ...
       CASE WHEN rr.format_id IS NOT NULL THEN
         rr.quantite_par_format * CASE rr.quantite_par_format_unite
           WHEN 'g' THEN 1
           WHEN 'kg' THEN 1000
           WHEN 'ml' THEN COALESCE(rr.format_densite, 1)
           WHEN 'l' THEN COALESCE(rr.format_densite, 1) * 1000
           ELSE 1
       ...
       CASE WHEN pr.quantite_brute IS NOT NULL AND rr.format_id IS NOT NULL
              AND rr.quantite_par_format > 0 AND rr.planned_quantity > 0 THEN
         (pr.quantite_brute * 1000) / (rr.quantite_par_format
...
COMMENT ON VIEW v_recipe_format_cost IS
  'mig 269 (audit A8) : renommage quantite_par_format_g -> quantite_par_format.';
