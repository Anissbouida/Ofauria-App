    ...
    const result = await db.query(
      `WITH ii_lines AS (
       ...
       SELECT
         ...
         s.name            AS supplier_name,
         po.order_number   AS purchase_order_number,
         -- Categorie affichee : on prefere la categorie de l'ingredient
         -- (granulaire : farines / produits_laitiers / ...) plutot que celle
         -- de l'expense_category de la facture (souvent juste "Matieres premieres").
